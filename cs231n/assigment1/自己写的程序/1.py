...
def compute_loss(A, Y, parameters, regRate):  #计算loss
    ...
    totalLoss = np.squeeze(totalLoss)
    return totalLoss
...
def pred(testSet, testLabel):  #在测试集上跑
    import pickle
    f = open(r'E:\python编程处\cs231n的作业\storeParameters_TwoLayers.txt', 'rb')
...
def storeParameters(parameters):  #存储参数
    import pickle
    f = open(r'E:\python编程处\cs231n的作业\storeParameters_TwoLayers.txt', 'wb')
