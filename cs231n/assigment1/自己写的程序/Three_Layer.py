...
def get_All_Data():  #加载图片数据集，并且划分为训练集和测试集（首先并没有划分dev集）
    ...
    return trainSet/255, trainLabel, testSet/255, testLabel, labelNames
...
def forward(X, parameters):  #softmax前向传播
    ...
    cache = (A1, W1, Z1, A2, W2, Z2, A3, W3, Z3)  #保存这些值便于backprop计算
    return A3, cache
...
def compute_loss(A, Y, parameters, regRate):  #计算loss
    ...
    for i in range(int(len(parameters)/2)):
        regLoss += regRate*np.sum(np.square(parameters['W'+str(i+1)]))
        
    totalLoss = dataLoss + regLoss
    return totalLoss

def backprop(X, A3, Y, cache, parameters, regRate):  #计算反向传播
    m = Y.shape[0]
    
    A3[Y.T, np.arange(m)] -= 1  #将对应的元素-1，即完成了梯度计算
    dZ3 = A3.copy()   #复制即可
    #dZ2 = softmax_backward(A2, Y)   
    A1, W1, Z1, A2, W2, Z2, A3, W3, Z3 = cache
    
    dW3 = np.dot(dZ3, A2.T)/m  + 2*regRate*W3
    db3 = np.sum(dZ3, axis = 1, keepdims = True)/m  #用keepdims防止假维度
    dA2 = np.dot(W3.T, dZ3)
...
def model(trainSet, trainLabel, layer_dims, learning_rate = 0.01, regRate = 0, batchSize = 256,
    ...
    for i in range(num_epoches):
        ...
        for minibatch in mini_batches:
            ...
            #前向传播
            A3, cache = forward(minibatch_X, parameters)
            #计算成本
            loss = compute_loss(A3, minibatch_Y, parameters, regRate)
            #反向传播
            grads = backprop(minibatch_X, A3, minibatch_Y, cache, parameters, regRate)
            #更新参数
...
def pred(testSet, testLabel):  #在测试集上跑
    import pickle
    f = open(r'E:\python编程处\cs231n的作业\storeParameters_ThreeLayers.txt', 'rb')
    parameters = pickle.load(f)
    A3, cache = forward(testSet, parameters)
    
    maxCol = np.max(A3, axis = 0).T
    m = testLabel.shape[0]
    yhat = np.zeros((m, 1))
    for i in range(m):
        ind = np.nonzero(A3[:, i] == float(maxCol[i]))[0]
        yhat[i] = ind
    return yhat

def storeParameters(parameters):  #存储参数
    import pickle
    f = open(r'E:\python编程处\cs231n的作业\storeParameters_ThreeLayers.txt', 'wb')
...
m, n = trainSet.shape
layer_dims = (n, 30, 20, 10)

#训练
parameters = model(trainSet, trainLabel, layer_dims, learning_rate = 0.05, regRate = 0, batchSize = 256,\
                    num_epoches = 1000)
