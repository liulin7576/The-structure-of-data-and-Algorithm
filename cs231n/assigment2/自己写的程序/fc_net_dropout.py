...
def forward(X, parameters, keep_prob = 0.8):  #softmax前向传播
    ...
    for i in range(1, L):
        ...
        A = Relu(Z)
        D = (np.random.rand(A.shape[0], A.shape[1]) < keep_prob) #每次失活20%
        A = np.multiply(D, A)/keep_prob    #使得理论的期望值不变
        caches.append((A_prev, D, parameters['W'+str(i)], Z))
...
def backprop(X, AL, Y, caches, regRate, keep_prob = 0.8):  #计算反向传播
    m = Y.shape[0]
    ...
    for i in range(L, 0, -1):
        if i == L:
            ...
            dZ = relu_backward(dA, (A_prev, W, Z))
        A_prev = A  #A_prev是为了用来计算Relu的反向微分
...
def model(trainSet, trainLabel, layer_dims, learning_rate = 0.05, regRate = 0, batchSize = 256,\
          keep_prob = 0.8, num_epoches = 1000):  #总的模型
    ...
    for i in range(num_epoches):
        ...
        for minibatch in mini_batches:
            ...
            #前向传播
            AL, caches = forward(minibatch_X, parameters, keep_prob)
            #计算成本
            loss = compute_loss(AL, minibatch_Y, parameters, regRate)
            #反向传播
            grads = backprop(minibatch_X, AL, minibatch_Y, caches, regRate, keep_prob)
            #更新参数
...
def pred(testSet, testLabel, parameters = None):  #在测试集上跑
    ...
    AL, caches = forward(testSet, parameters, keep_prob = 1)
...
trainSet, trainLabel =  trainSet[:10000], trainLabel[:10000]  #选取小的测试集，用来验证模型是否正确，正确的话能够过拟合
m, n = trainSet.shape
layer_dims = [n, 100, 100, 10]

#parameters = model(trainSet, trainLabel, layer_dims, learning_rate = 0.00011, regRate = 0, batchSize = 256,   #使用dropOut，所以这里的regRate置0就好了
#                   keep_prob = 0.8, num_epoches = 500)
'''
